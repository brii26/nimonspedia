...
const Navbar = () => {
  ...
  const isActive = (path: string) => currentPath === path ? 'text-[#42b549] bg-[#edfef0] font-semibold' : 'text-gray-600 hover:text-[#42b549] hover:bg-gray-50';
  ...
  return (
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-[1000] shadow-sm">
      <div className="max-w-[1200px] mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center flex-shrink-0 order-2 lg:order-none">
            <a href="/" className="flex items-center gap-2 text-[#42b549] no-underline" aria-label="Nimonspedia Home">
              <img 
              ...
              />
              <span className="text-xl font-bold text-[#42b549]">Nimonspedia</span>
            </a>
          </div>
                  ...
                  {flags.checkout_enabled && (
                    <a href="/cart" className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-all relative ${isActive('/cart')}`}>
                      <span className="text-lg relative">
                        🛒
                        {cartCount > 0 && (
                            <span className="absolute -top-2 -right-2 bg-[#42b549] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center border-2 border-white">
                                {cartCount}
                            </span>
                        )}
                      </span>
            ...
            {user && (
              <div className="flex items-center gap-4">
                ...
                <div className="relative">
                  ...
                  >
                    <div className="w-9 h-9 rounded-full bg-[#42b549] flex items-center justify-center text-white font-bold text-sm overflow-hidden shadow-sm">
                      {user.avatar ? (
      ...
      {/* Mobile Menu */}
      <div 
        className={`fixed inset-y-0 left-0 w-[75%] max-w-xs bg-gradient-to-b from-[#42b549] to-[#2d8f34] shadow-2xl transform transition-transform duration-300 ease-in-out z-[1002] lg:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="flex flex-col h-full p-6 text-white overflow-y-auto">
            ...
            {user && (
                <div className="flex items-center gap-4 mb-8 pb-6 border-b border-white/20">
                    <div className="w-12 h-12 rounded-full bg-white text-[#42b549] flex items-center justify-center font-bold text-lg shadow-md">
                        {user.avatar ? (
                    ...
                    <div>
                        ...
                        <div className="text-sm text-[#c8f0ca]">{user.role}</div>
                    </div>
                </div>
            )}
